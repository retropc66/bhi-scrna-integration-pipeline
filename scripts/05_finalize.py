...
parser.add_argument('--method', type=str, required=True, 
                    choices=['scvi', 'scanvi', 'mrvi', 'sysvi'],
...
if not np.array_equal(obs_names_embed, adata.obs_names.to_numpy()):
    print("   ⚠️  Warning: Cell order mismatch, reindexing...")
...
# Compute UMAP
print(f"\n🗺️  Computing UMAP (min_dist={UMAP_MIN_DIST})...")
...
print(f"\nContents:")
print(f"   • X: Normalized, log-transformed counts ({adata.n_obs:,} × {adata.n_vars:,})")
...
print(f"\n💡 Next steps:")
print(f"   • Visualize: scanpy.pl.umap(adata, color=['celltype_pred', 'leiden', 'sample_id'])")
print(f"   • Explore: Use Jupyter notebook or scanpy for downstream analysis")
print(f"   • Re-cluster: Run with different --leiden-res if needed")
